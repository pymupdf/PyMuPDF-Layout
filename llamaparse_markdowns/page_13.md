...
$$a_k \text{ is defined to be } a_{(k \bmod p)} \text{ for all } k \notin \{0, ..., p-1\} \quad (a_{p-1} = 0, \text{ trivially}).$$  
And so  
$$\sum_{k=0}^{p-2} (a_{p-k} - a_1) \zeta^k = u = \zeta^{-t} u = \sum_{k=0}^{p-2} (a_{k+t} - a_{(p-1)+t}) \zeta^k$$  
...
Now by the proof of 1, there is a unique $c$ modulo $p$ such that $\zeta^c u$ is primary, and this $c$ is the solution to  
$$ax \equiv b \pmod{p}$$  
where $u \equiv a + b \lambda$ ($\lambda^2$) where $\lambda = (1 - \zeta)$. Now  
$$u = \sum_{k=0}^{p-2} a_k \zeta^k.$$  

Writing, as a polynomial,  
$$f(x) = \sum_{k=0}^{p-2} a_k x^k,$$  
we can find $a$ and $b$ by finding the coefficients of 1 and $x$ respectively of $f(1-x)$ since $\zeta = 1 - \lambda$. Making elementary use of the Binomial Theorem, we see that  
$$f(1-x) = \sum_{k=0}^{p-2} a_k (1-x)^k = \sum_{k=0}^{p-2} a_k - \sum_{k=0}^{p-2} k a_k x + \ldots$$  
...
Now  
$$k_0 + t \equiv p - k_0 \pmod{p} \implies k_0 + t \equiv -(k_0 + t) + t \pmod{p} \implies (k_0 + t) \equiv 2^{-1} t \implies b_{k_0 + t} = a_{k_0 + t} = a_{2^{-1} t}.$$  

Finally, note that for $a_i = a_{t - i} = b_l$ for $1 \leq l \leq \frac{p-1}{2}$ by (1.2),  
$$i a_i + (t - i) a_{t - i} = t b_l.$$
