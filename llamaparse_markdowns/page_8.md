...
where \( U \) and \( V \) are orthogonal matrices that span the range and the null space of \( C_{ij} \) and \( D_{kl} = \lambda^2 \delta_{kl} \), a diagonal matrix with singular values along the diagonal. In doing the SVD, we select the dominant modes to contribute to the \(\chi^2\) by requiring that

$$
\lambda^2 > \sqrt{\frac{2}{N_{jk}}}.
$$
