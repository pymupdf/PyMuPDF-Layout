...
$$
\rho^{XABE} \equiv \sum_x p_X(x) \ket{x}\bra{x}^X \otimes U^{A' \to BE}_{\mathcal{N}}(\phi_x^{AA'}),
$$

$$
\theta^{XABE} \equiv \sum_x p_X(x) \ket{x}\bra{x}^X \otimes U^{A' \to BE}_{\mathcal{N}}(\varphi_x^{AA'}),
$$

$U^{A' \to BE}_{\mathcal{N}}$ is an isometric extension of $\mathcal{N}$, $\varphi_x^{A'} = \Delta(\phi_x^{A'})$, and $\Delta$ is the completely dephasing channel.
...
$$
\sigma^{XAYE} \equiv \sum_x p_X(x) \ket{x}\bra{x}^X \otimes (\Delta^{B \to Y} \circ U^{A' \to BE}_{\mathcal{N}})(\phi_x^{AA'}).
$$
...
$$
\frac{1}{2} \ket{0}\bra{0}^X \otimes \psi_0^{AA'} + \frac{1}{2} \ket{1}\bra{1}^X \otimes \psi_1^{AA'}, \tag{D1}
$$

where $\psi_0^{AA'}$ and $\psi_1^{AA'}$ are pure states, defined as follows for $\mu \in [0, 1/2]$:

$$
\mathrm{Tr}_A \left\{ \psi_0^{AA'} \right\} = \mu \ket{0}\bra{0}^{A'} + (1 - \mu)\ket{1}\bra{1}^{A'}, \tag{D2}
$$

$$
\mathrm{Tr}_A \left\{ \psi_1^{AA'} \right\} = (1 - \mu)\ket{0}\bra{0}^{A'} + \mu \ket{1}\bra{1}^{A'}. \tag{D3}
$$
