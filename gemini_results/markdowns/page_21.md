...
$$
\sup_{\{a_k\}_{k=-J}^{J} \in \mathcal{S}_{\delta}^{J,K}(\widehat{y})} \sum_{k=-J}^{J} \max \{ F_1(a_{k+1}) - F_0(a_k), 0 \}, \tag{B.2}
$$

where

$$
\mathcal{S}_{\delta}^{J,K}(y) = \left\{ \{a_k\}_{k=-J}^{J} \, ; \, \begin{aligned} & a_J \le y + K\delta, \, a_{-J} \ge y - K\delta, \, 0 \le a_{k+1} - a_k \le \delta, \\ & \delta < a_{k+2} - a_k \text{ for each integer } k \end{aligned} \right\},
$$

$$
\widehat{y} = \arg \max_{y \in \{y^* + k\delta\}_{k=-\infty}^{\infty}} \sum_{k=-K}^{K} \max \left( F_1 \left( y + (k+1)\delta \right) - F_0 \left( y + k\delta \right), 0 \right).
$$
