...
$$
\rho(E) = \left( \frac{\partial E}{\partial k} \right)^{-1} = \begin{cases} \frac{1}{\sqrt{4\kappa_0^2 - E^2}} & -2\kappa_0 < E < 2\kappa_0 \\ 0 & |E| > 2\kappa_0 \end{cases} \tag{48}
$$
...
$$
V(E) = \begin{cases} \frac{\kappa_a^2}{\pi \kappa_0} \sqrt{1 - \left(\frac{E}{2\kappa_0}\right)^2} & -2\kappa_0 < E < 2\kappa_0 \\ 0 & |E| > 2\kappa_0 \end{cases} \tag{49}
$$

which is non-singular. Substitution of Eq. (49) into Eq. (11) yields the following expression for the self-energy $\Sigma(z)$ [46]

$$
\Sigma(z) = -i \frac{\kappa_a^2}{2\kappa_0^2} \left( \sqrt{4\kappa_0^2 - z^2} + iz \right) \tag{50}
$$
...
$$
\xi^2 + \frac{E_a}{\kappa_0}\xi + 1 - (\kappa_a/\kappa_0)^2 = 0. \tag{52}
$$

Then the Hamiltonian $H$ has a real-valued energy spectrum if and only if $|\xi_{1,2}| \le 1$. Figure 2 shows the domain in the plane $(\text{Im}(E_a)/\kappa_0, \kappa_a/\kappa_0)$ where $H$ has a purely continuous energy spectrum for a few increasing values of the ratio $|\text{Re}(E_a)/\kappa_0|$. The domain lies in the sector $\kappa_a/\kappa_0 \le \sqrt{2}$ and shrinks toward $\text{Im}(E_a)/\kappa_0 = \kappa_a/\kappa_0 = 0$ as $|\text{Re}(E_a)/\kappa_0| \to 2^-$. For $|\text{Re}(E_a)/\kappa_0| \le 2$, bound states do exist for any value of $\kappa_a/\kappa_0$ and $\text{Im}(E_a)/\kappa_0$. The wider domain is attained for $\text{Re}(E_a) = 0$. In particular, for $\text{Re}(E_a) = 0$ and $\kappa_a/\kappa_0 = \sqrt{2}$, from Eq. (52) it follows that $H$ has a real-valued energy spectrum provided that

$$
-2\kappa_0 < \text{Im}(E_a) < 2\kappa_0. \tag{53}
$$

![Figure 2: Domains of non-existence of bound states](figure-2-placeholder.png)

**Figure 2:** Domains of non-existence of bound states for the Hamiltonian $H$ in the $(\text{Im}(E_a)/\kappa_0, \kappa_a/\kappa_0)$ plane (shaded regions) for increasing values of the ratio $|\text{Re}(E_a)|/\kappa_0$. For a non-Hermitian Hamiltonian, i.e. $\text{Im}(E_a) \ne 0$, in the shaded regions the energy spectrum of $H$ is real-valued and purely continuous. Spectral singularities occur at the boundary of the shaded regions.
...
$$
\text{Im}(E_a) = \pm \frac{\kappa_a^2}{\kappa_0} \sqrt{1 - \left( \frac{\mathcal{E}_0}{2\kappa_0} \right)^2} \tag{54}
$$

$$
\text{Re}(E_a) = \left( 1 - \frac{\kappa_a^2}{2\kappa_0^2} \right) \mathcal{E}_0. \tag{55}
$$
