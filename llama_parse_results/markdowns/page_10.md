...
where $\frac{t_0-t_W}{t_1-t_0} \delta +w \leq b_{k+1} \leq b_k$. Consequently, the sharp upper bound is written as follows: letting $F_{\Delta,W}^U (\delta|w)$ be the sharp upper bound on $\text{Pr} (Y_1 - Y_0 \leq \delta|W = w)$ ,
...
$$\{a_k\}_{k=-\infty}^{\infty} \in \arg \max_{\{a_k\}_{k=-\infty}^{\infty} \in A_\delta} \sum_{k=-\infty}^{\infty} \max \{F_1 (a_{k+1}) - F_0 (a_k) , 0\} ,$$

where $A_\delta = \{\{a_k\}_{k=-\infty}^{\infty} : 0 \leq a_{k+1} - a_k \leq \delta \text{ for each integer } k\}$ .

It is innocuous to assume that $\{a_k\}_{k=-\infty}^{\infty}$ satisfies $a_{k+2} - a_k > \delta$ for each integer $k$.

**Proof.** I will show that for any sequence $\{a_k\}_{k=-\infty}^{\infty} \in A_\delta$ satisfying $a_{k+2} - a_k \leq \delta$ for some integer $k$, one can construct $\{\tilde{a}_k\}_{k=-\infty}^{\infty} \in A_\delta$ with $\tilde{a}_{k+2} - \tilde{a}_k > \delta$ for each integer $k$ and
