...
$$a_k$$ is defined to be $$a_{(k \bmod p)}$$ for all $$k \notin \{0, ..., p-1\}$$ ($$a_{p-1} = 0$$, trivially). And so $$\sum_{k=0}^{p-2}(a_{p-k} - a_1)\zeta^k = \overline{u} = \zeta^{-t}u = \sum_{k=0}^{p-2}(a_{k+t} - a_{(p-1)+t})\zeta^k$$ by 1 and therefore, since this representation is unique, we get

(1.1) $$a_{k+t} - a_{(p-1)+t} = a_{p-k} - a_1$$ for all $$0 \leq k \leq p-1$$
...
Now by the proof of 1, there is a unique $$c$$ modulo $$p$$ such that $$\zeta^c u$$ is primary, and this $$c$$ is the solution to $$ax \equiv b$$ $$(p)$$ where $$u \equiv a + b\lambda$$ $$(\lambda^2)$$ where $$\lambda = (1-\zeta)$$. Now $$u = \sum_{k=0}^{p-2} a_k \zeta^k$$. Writing, as a polynomial, $$f(x) = \sum_{k=0}^{p-2} a_k x^k$$, we can find $$a$$ and $$b$$ by finding the coefficients of 1 and $$x$$ respectively of $$f(1-x)$$ since $$\zeta = 1 - \lambda$$. Making elementary use of the Binomial Theorem, we see that $$f(1-x) = \sum_{k=0}^{p-2} a_k(1-x)^k = \sum_{k=0}^{p-2} a_k - \sum_{k=0}^{p-2} ka_k x + ...$$ (we only need the first two terms). So $$c$$ is the solution to
