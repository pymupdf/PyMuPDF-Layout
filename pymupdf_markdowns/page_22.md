...
III. B OUNDS ON THE N ETWORK T HROUGHPUT
...
user that maximizes Q n,k,l where:


Q n,k,l = [p] [n,k,l] [h] [n,k,l] (9)

ξ n,l + σ [2]


Thus, Q n,k,l is an SINR term for each user k at each subcarrier n in each cell l assuming worst


case interference. We collect these SINR terms into a vector q n,l = [q n,1,l, q n,2,l ...., q n,K,l ] and


then stack all the vectors in a matrix Q l ∈ R [N] [×][K] . The resulting allocations based on this criteria


are then used to compute the LB network throughput using (1).


Note that if ξ n,l = 0, than Q n,k,l becomes the marginal rate which is shown to be a near-optimal
