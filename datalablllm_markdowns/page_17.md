...
**Example 3** (Roy Model) In the Roy model, individuals self-select into treatment when their benefits from the  $\mathbf{S}$ treatment are greater than nonpecuniary costs for treatment participation. The extended Roy model assumes that the nonpecuniary cost is deterministic with the following selection equation:

$$D = \mathbf{1} \{ Y_1 - Y_0 \ge \mu_C \left( Z \right) \},$$
...
$$\Pr\left\{Y_{1} - Y_{0} \le \delta \middle| (Y_{0}, Y_{1}) \in C\right\}.$$