...
$$\{a_k\}_{k=-\infty}^{\infty} \in \mathop{\operatorname{arg\,max}}\_{\{a_k\}_{k=-\infty}^{\infty} \in \mathcal{A}\_{\delta}} \sum
\limits
\_{k=-\infty}^{\infty} \max \{F_1(a
\_{k+1}) - F_0(a_k), 0\},\$$
where 

 $\mathcal{A}\_{\delta} = \{\{a_k\}
\_{k=-\infty}^{\infty} ; 0 \le a
\_{k+1} - a_k \le \delta \text{ for each integer } k\}$ 
