...
= \int \left\{1 - \sup_{\{B_k\}_{k=-\infty}^\infty} \sum_{k=-\infty}^\infty \max \{\mu_{0,w} (B_k|w) - \mu_{1,w} (B_k^D|w), 0\}\right\} dF_W (w) \\
= 1 + \int \inf_{\{b_k\}_{k=-\infty}^\infty} \sum_{k=-\infty}^\infty \min \left\{ F_{1,W} \left( \frac{t_1 - t_w}{t_0 - t_w} b_{k+1} - \frac{t_1 - t_0}{t_0 - t_w} w|w \right) - F_{0,w} (b_k|w), 0 \right\} dF_W (w)
$$

where $\frac{t_0-t_w}{t_1-t_0} \delta + w \le b_{k+1} \le b_k$. $\blacksquare$
...
$$
\{a_k\}_{k=-\infty}^\infty \in \arg \max_{\{a_k\}_{k \in \mathcal{A}_\delta}^\infty} \sum_{k=-\infty}^\infty \max \{F_1 (a_{k+1}) - F_0 (a_k),0\}, \\
\text{where } \mathcal{A}_\delta = \{\{a_k\}_{k=-\infty}^\infty; 0 \le a_{k+1} - a_k < \delta \text{ for each integer } k\}.
$$

It is innocuous to assume that $\{a_k\}_{k=-\infty}^\infty$ satisfies $a_{k+2} - a_k > \delta$ for each integer $k$.

**Proof.** I will show that for any sequence $\{a_k\}_{k=-\infty}^\infty \in \mathcal{A}_\delta$ satisfying $a_{k+2} - a_k < \delta$ for some integer $k$, one can construct $\{\tilde{a}_k\}_{k=-\infty}^\infty \in \mathcal{A}_\delta$ with $\tilde{a}_{k+2} - \tilde{a}_k > \delta$ for each integer $k$ and
