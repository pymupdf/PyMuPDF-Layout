This graph shows the cumulative IV (initialization vector) tokens found as additional users are downloaded using different queue ordering strategies. The predictive ranking strategy (solid line) significantly outperforms both the random baseline (dashed line) and edge count ranking (dotted line), finding approximately 1.25 million IV tokens after downloading 500 additional users. The other two strategies show much slower, more linear growth patterns with similar performance to each other.
...
of 10,785 Twitter users. (5,684 users came from our search predictive ranking model, the edge count ranking is worse

download a small set of users and use that as training data high edge count connections from Bengali speaking Twitter
...
download queue for each of the three prioritization strategies. language models used the data from the 300 users in the
