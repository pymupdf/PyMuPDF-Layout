...
\( \Delta n=0.115 \frac{\sqrt[4]{\sigma}}{(W / S)} V_{e}(K U) \)

where

\( \Delta n= \) change in load factor due to gust
\( m= \) lift curve slope, unit of \( C_{L} \) per degree of \( \alpha \)
\( \sigma= \) altitude density ratio
\( W / S= \) wing loading, psf
\( V_{e}= \) equivalentairspeed, knots
\( K U= \) equivalent sharp edged gust velocity ft. per sec.
