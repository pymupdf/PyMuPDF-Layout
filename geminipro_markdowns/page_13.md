...
Now by the proof of 1, there is a unique $c$ modulo $p$ such that $\zeta^c u$ is primary, and this $c$ is the solution to $ax \equiv b \pmod p$ where $u \equiv a + b\lambda \pmod{\lambda^2}$ where $\lambda = (1 - \zeta)$. Now $u = \sum_{k=0}^{p-2} a_k\zeta^k$. Writing, as a polynomial, $f(x) = \sum_{k=0}^{p-2} a_kx^k$, we can find $a$ and $b$ by finding the coefficients of 1 and $x$ respectively of $f(1 - x)$ since $\zeta = 1 - \lambda$. Making elementary use of the Binomial Theorem, we see that $f(1 - x) = \sum_{k=0}^{p-2} a_k(1 - x)^k = \sum_{k=0}^{p-2} a_k - \sum_{k=0}^{p-2} ka_kx + \dots$ (we only need the first two terms). So $c$ is the solution to
