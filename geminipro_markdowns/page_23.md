...
$$
E(k) = -2\kappa_0 \cos k \;, \ v(k) = -\sqrt{\frac{2}{\pi}}\kappa_a \sin k \tag{47}
$$
...
![Figure 2: Domains of non-existence of bound states for the Hamiltonian H](figure-2.png)

**Figure 2 (summary):** Four plots showing the domains of non-existence of bound states in the $(\text{Im}(E_a)/\kappa_0, \kappa_a/\kappa_0)$ plane (shaded regions).
- Axes: Y-axis is $\kappa_a/\kappa_0$ (0 to 1.6), X-axis is $\text{Im}(E_a)/\kappa_0$ (-2 to 2).
- Panels correspond to increasing values of $|\text{Re}(E_a)/\kappa_0|$:
  - Top-left: $\text{Re}(E_a)/\kappa_0 = 0$. A wide central shaded cone.
  - Top-right: $\text{Re}(E_a)/\kappa_0 = \pm 0.5$. The cone narrows slightly.
  - Bottom-left: $\text{Re}(E_a)/\kappa_0 = \pm 1$. The cone splits or narrows significantly.
  - Bottom-right: $\text{Re}(E_a)/\kappa_0 = \pm 1.8$. Very narrow regions.
- **Caption:** Domains of non-existence of bound states for the Hamiltonian $H$ in the $(\text{Im}(E_a)/\kappa_0, \kappa_a/\kappa_0)$ plane (shaded regions) for increasing values of the ratio $|\text{Re}(E_a)|/\kappa_0$. For a non-Hermitian Hamiltonian, i.e. $\text{Im}(E_a) \neq 0$, in the shaded regions the energy spectrum of $H$ is real-valued and purely continuous. Spectral singularities occur at the boundary of the shaded regions.
