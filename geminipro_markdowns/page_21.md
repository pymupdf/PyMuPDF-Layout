...
$$
\sup_{0 \leq y \leq \delta} \sum_{k=-\infty}^{\infty} \max \left( F_1 (y + (k + 1) \delta) - F_0 (y + k\delta), 0 \right), \tag{B.1}
$$
...
$$
V (\delta) = \sup_{0 \leq y \leq \delta} \sum_{k=-\infty}^{\infty} \max \left( F_1 (y + (k + 1) \delta) - F_0 (y + k\delta), 0 \right),
$$
...
where $y^* \in \arg \max_{0 \leq y \leq \delta} \sum_{k=-\infty}^{\infty} \max \left( F_1 (y + (k + 1) \delta) - F_0 (y + k\delta), 0 \right)$ and $K$ is a nonnegative integer.
...
$$
\mathcal{S}_{\delta}^{J,K} (y) = \left\{ \begin{aligned} &\{a_k\}_{k=-J}^J ; \, a_J \leq y + K\delta, \, a_{-J} \geq y - K\delta, \, 0 \leq a_{k+1} - a_k \leq \delta, \\ &\delta < a_{k+2} - a_k \text{ for each integer } k \end{aligned} \right\},
$$

$$
\hat{y} = \arg \max_{y \in \{y^*+k\delta\}_{k=-\infty}^{\infty}} \sum_{k=-K}^{K} \max \left( F_1 (y + (k + 1) \delta) - F_0 (y + k\delta), 0 \right).
$$
