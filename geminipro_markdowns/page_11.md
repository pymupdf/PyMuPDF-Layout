...
$$
C(t) + \lambda Q(t) \geq C(s) + \lambda Q(s). \tag{C1}
$$
...
$$
\begin{aligned}
\rho^{XABE} &\equiv \sum_x p_X (x) \ket{x}\bra{x}^X \otimes U_{\mathcal{N}}^{A' \to BE} (\phi_x^{AA'}), \\
\theta^{XABE} &\equiv \sum_x p_X (x) \ket{x}\bra{x}^X \otimes U_{\mathcal{N}}^{A' \to BE} (\varphi_x^{AA'}),
\end{aligned}
$$
...
$$
\sigma^{XAYE} \equiv \sum_x p_X (x) \ket{x}\bra{x}^X \otimes (\Delta^{B \to Y} \circ U_{\mathcal{N}}^{A' \to BE}) (\phi_x^{AA'}).
$$
...
$$
\text{Tr}_A \left\{ \psi_0^{AA'} \right\} = \mu \ket{0}\bra{0}^{A'} + (1 - \mu) \ket{1}\bra{1}^{A'} , \tag{D2}
$$

$$
\text{Tr}_A \left\{ \psi_1^{AA'} \right\} = (1 - \mu) \ket{0}\bra{0}^{A'} + \mu \ket{1}\bra{1}^{A'} . \tag{D3}
$$
