...
## Lemma 11
*λ parametrizes all points on the CQ and CE trade-off curves with the possible exception of those lying on segments of constant slope.*
...
$$
f_\lambda(\mathcal{N}) = \max_{\rho^{XBE}} I(X; B) + \lambda I(A\rangle BX).
$$

For any $0 \le t \le 1$, we have $f_\lambda(\mathcal{N}) = C(t) + \lambda Q(t)$ if and only if

$$
C(t) + \lambda Q(t) \ge C(s) + \lambda Q(s).
\quad (C1)
$$

for all $0 \le s \le 1$. Perhaps more instructively, if $s < t$ and $Q(s) \ne Q(t)$, this inequality can be written as
...
## Lemma 12
*Let $\mathcal{N}$ be a generalized dephasing channel. In the optimization task for the CQ trade-off curve, it suffices to consider a classical-quantum state with diagonal conditional density operators, in the sense that the following inequality holds:*

$I(X; B)_\rho + \lambda I(A\rangle BX)_\rho \le I(X; B)_\theta + \lambda I(A\rangle BX)_\theta,$

*where*

$$
\rho^{XABE} = \sum_x p_x(x) |x\rangle\langle x|^X \otimes U_{\mathcal{N}}^{A' \to BE}(\phi_x^{AA'}),
$$
$$
\theta^{XABE} = \sum_x p_x(x) |x\rangle\langle x|^X \otimes U_{\mathcal{N}}^{A' \to BE}(\varphi_x^{AA'}),
$$

*$U_{\mathcal{N}}^{A' \to BE}$ is an isometric extension of $\mathcal{N}$, $\varphi_x^{A'} = \Delta(\phi_x^{A'})$, and $\Delta$ is the completely dephasing channel.*
...
$$
\sigma^{XAYE} = \sum_x p_x(x) |x\rangle\langle x|^X \otimes (\Delta^{B \to Y} \circ U_{\mathcal{N}}^{A' \to BE})(\phi_x^{AA'}).
$$
...
$$
\begin{aligned}
I(X; B)_\rho + \lambda I(A\rangle BX)_\rho &= H(B)_\rho + (\lambda - 1)H(B|X)_\rho - \lambda H(E|X)_\rho \\
...
## Lemma 13
*An ensemble of the following form parametrizes all points on the CQ trade-off curve for a qubit dephasing channel:*
$$
\frac{1}{2}|0\rangle\langle 0|^X \otimes \psi_0^{AA'} + \frac{1}{2}|1\rangle\langle 1|^X \otimes \psi_1^{AA'}
\quad (D1)
$$
*where $\psi_0^{AA'}$ and $\psi_1^{AA'}$ are pure states, defined as follows for $\mu \in [0, 1/2]$:*

$$
\mathrm{Tr}_A\{\psi_0^{AA'}\} = \mu |0\rangle\langle 0|^{A'} + (1 - \mu)|1\rangle\langle 1|^{A'}
\quad (D2)
$$
$$
\mathrm{Tr}_A\{\psi_1^{AA'}\} = (1 - \mu)|0\rangle\langle 0|^{A'} + \mu|1\rangle\langle 1|^{A'}
\quad (D3)
$$
